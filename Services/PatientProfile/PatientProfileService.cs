...
public class PatientProfileService : IPatientProfileService
{
    ...
    public async Task<ActionResult<PagedResponse<PatientNotificationResponse>>> GetNotificationsAsync(
        string? status,
        int page,
        int pageSize,
        ClaimsPrincipal user,
        CancellationToken cancellationToken = default)
    {
        ...
        var query = _dbContext.Notifications
            .AsNoTracking()
            .Where(notification => notification.UserId == patientId.Value);
        ...
        var notifications = await query
            ...
            .Take(pageSize)
            .Select(notification => new PatientNotificationResponse
            {
                NotificationId = notification.NotificationId,
                Title = notification.Title,
                IsRead = notification.IsRead,
                Category = InferNotificationCategory(notification.Title)
            })
            .ToListAsync(cancellationToken);

        return new OkObjectResult(BuildPagedResponse(notifications, totalCount, page, pageSize));
    }

    public async Task<ActionResult<PatientNotificationResponse>> MarkNotificationAsReadAsync(
        Guid notificationId,
        ClaimsPrincipal user,
        CancellationToken cancellationToken = default)
    {
        ...
        var notification = await _dbContext.Notifications
            .FirstOrDefaultAsync(
                x => x.NotificationId == notificationId && x.UserId == patientId.Value,
                cancellationToken);
        ...
        return new OkObjectResult(new PatientNotificationResponse
        {
            NotificationId = notification.NotificationId,
            Title = notification.Title,
            IsRead = notification.IsRead,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var notifications = await _dbContext.Notifications
            .Where(notification => notification.UserId == patientId.Value && !notification.IsRead)
            .ToListAsync(cancellationToken);
        ...
        await _dbContext.SaveChangesAsync(cancellationToken);
        return new OkObjectResult(new { updated = notifications.Count });
    }
    ...
    private static string InferNotificationCategory(string title)
    {
        ...
        if (title.Contains("lab", StringComparison.OrdinalIgnoreCase)
